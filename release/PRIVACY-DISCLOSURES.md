...
This document describes the intended version 1.0 binary after the current release changes. Re-check the final archived binaries before answering store questionnaires.
...
If server-side receipt validation is enabled, disclose Purchases under App Functionality and Fraud Prevention as required by the final validator configuration.
...
- Android `INTERNET`: Store communication and optional receipt validation
- Android `VIBRATE`: Core tactile feedback
- iOS: No protected-resource permission prompts
